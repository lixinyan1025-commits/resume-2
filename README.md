...
- 飘落效果由花瓣进一步调整为柳絮：本地 SVG 绘制柔软絮丝，配合缓慢摇曳与远近柔焦；均匀穿过整个画面，允许短暂掠过文字，但不拦截点击或滑动。上限仍为桌面 38 片、手机 20 片。后台暂停生成，系统开启减少动态效果时立即清除柳絮。
...
script.js     打字、飘絮、证书弹窗、访问统计和懒加载交互
assets/       图片、证书原图与首屏视频
```
...
- 页面尊重系统的“减少动态效果”设置，会关闭视频运动、逐字动画和柳絮飘落。
