...
- 米白与淡粉花瓣飘落、纵向流水文字、减少动态效果支持和响应式布局
- 页面自身不播放音乐；进入七夕展示后由嵌入网页管理其音乐

## 2026-09-20 更新

- 删除“PPT 制作”兴趣卡片，重新平衡七张兴趣卡片的栅格。
- 将竹叶替换为米白、淡粉花瓣，用轻微横向漂移、翻转和远近层次表现飘落；均匀穿过整个画面，允许短暂掠过文字，但不拦截点击或滑动。上限仍为桌面 38 片、手机 20 片。后台暂停生成，系统开启减少动态效果时立即清除花瓣。
- 自我介绍按本人新提供的原文替换，只调整分段；取消所有下划线和荧光笔标记，改用开场句、正文、感想与结语的字号、色彩和留白层次。
- 实现前核查了 [Jeroen Hammann / Sakura.js](https://github.com/jhammann/sakura)（MIT 许可），参考轻量 CSS 飘花方向；未复制该库代码或引入依赖，花瓣形状与运动由本项目现有代码调整实现。
...
script.js     打字、飘花、证书弹窗、访问统计和懒加载交互
assets/       图片、证书原图与首屏视频
```
...
- 页面尊重系统的“减少动态效果”设置，会关闭视频运动、逐字动画和花瓣飘落。
